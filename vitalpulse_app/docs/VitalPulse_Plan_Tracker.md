...
- [ ] `suspendUser`/`reactivateUser` Cloud Functions (replacing `db.js`'s client-side `suspendDonor`/`reactivateDonor`, still called directly from `admin.html`) — **urgent, not just "eventually":** confirmed via `firestore.rules.test.js` abuse-case tests (2026-07-25) that this exact client write is already rejected by the Phase 2 rules (writes `isAvailable`, which isn't in the `system_admin` privileged-update whitelist) — the admin "suspend donor" button will silently fail the moment these rules deploy. Deeper issue underneath: even a whitelist-widened version of this write would only touch a cosmetic Firestore field — it never calls `revokeRole`, so it wouldn't actually revoke the donor's real Firestore access (governed by the custom claim). The Cloud Function needs to do both atomically. `verifyHospital`/`rejectHospital` write only whitelisted fields and are NOT affected by this gap.
